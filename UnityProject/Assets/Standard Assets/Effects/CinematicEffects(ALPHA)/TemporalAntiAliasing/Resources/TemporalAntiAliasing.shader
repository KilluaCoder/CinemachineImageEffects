Shader "Hidden/Temporal Anti-aliasing"
{
    ...
    CGINCLUDE
        ...
        // Chroma manipulation idea inspired from Playdead's implementation
            ...
            float4 history = convertToYCoCg(tex2D(_HistoryTex, uv - motion.xy));
        #else
            float4 history = convertToYCoCg(getFilteredHistorySample(motion.xy));
        #endif
        ...
        float4 fragment(Varyings input) : SV_Target
        {
            ...
            float3 motion = float3(
                tex2D(_CameraMotionVectorsTexture, closestFragment.xy).xy * .5, closestFragment.z);

            return reproject(uv, motion);
        }
